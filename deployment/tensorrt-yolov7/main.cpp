...
void Yolov6::Init(char* model_path) {
  ...
  auto out_dims1 = engine->getBindingDimensions(engine->getBindingIndex("num_detections"));
  out_size1 = 1;
  for (int j = 0; j < out_dims1.nbDims; j++) {
    out_size1 *= out_dims1.d[j];
  }
  auto out_dims2 = engine->getBindingDimensions(engine->getBindingIndex("detection_boxes"));
  out_size2 = 1;
  for (int j = 0; j < out_dims2.nbDims; j++) {
    out_size2 *= out_dims2.d[j];
  }
  auto out_dims3 = engine->getBindingDimensions(engine->getBindingIndex("detection_scores"));
  out_size3 = 1;
  for (int j = 0; j < out_dims3.nbDims; j++) {
    out_size3 *= out_dims3.d[j];
  }
  auto out_dims4 = engine->getBindingDimensions(engine->getBindingIndex("detection_classes"));
  out_size4 = 1;
...
extern "C" Yolov6* __cdecl Init(char* model_path);
Yolov6* __cdecl Init(char* model_path) {
  Yolov6* yolov6 = new Yolov6();
  yolov6->Init(model_path);
  return yolov6;
}

extern "C" Yolov6* __cdecl Infer(
    Yolov6* yolov6,
    int aWidth,
    int aHeight,
    int aChannel,
...
extern "C" void __cdecl Destroy(Yolov6* yolov6);
void __cdecl Destroy(Yolov6* yolov6) {
  yolov6->Destroy();
}
...
int main(int argc, char** argv) {
  if (argc == 5 && std::string(argv[1]) == "-model_path" && std::string(argv[3]) == "-image_path") {
    ...
    std::cerr << "--> yolov7 -model_path ./output.trt -image_path ./demo.jpg" << std::endl;
    return -1;
  }
}
