...
from utils.general import box_xyxy_to_cxcywh, box_cxcywh_to_xyxy, torch_distributed_zero_first
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            self.frame += 1
            print(f'video {self.count + 1}/{self.nf} ({self.frame}/{self.nframes}) {Path(path).name}: ', end='')
            ...
            print(f'image {self.count}/{self.nf} {Path(path).name}: ', end='')
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if rank in [-1, 0]:
            pbar = tqdm(pbar)
        for i, file in pbar:
            label_i = self.labels[i]  # label
            if label_i is not None and label_i.shape[0]:
                assert label_i.shape[1] == 5, '> 5 label columns: %s' % file
                assert (label_i >= 0).all(), 'negative labels: %s' % file
                assert (label_i[:, 1:] <= 1).all(), 'non-normalized or out of bounds coordinate labels: %s' % file
                if np.unique(label_i, axis=0).shape[0] < label_i.shape[0]:  # duplicate rows
                    nd += 1  # print('WARNING: duplicate rows in %s' % self.label_files[i])  # duplicate rows
                if single_cls:
                    label_i[:, 0] = 0  # force dataset into single-class mode
                ...
                if create_datasubset and ns < 1E4:
                    ...
                    exclude_classes = 43
                    if exclude_classes not in label_i[:, 0]:
                        ns += 1
                ...
                if extract_bounding_boxes:
                    ...
                    for j, x in enumerate(label_i):
                        ...
                        b[2:] = b[2:] * 1.3 + 30  # pad
                        b = box_cxcywh_to_xyxy(b.reshape(-1, 4)).ravel().astype(np.int)
            ...
            try:
                ...
                if os.path.isfile(label):
                    with open(label, 'r') as f:
                        label_i = np.array([x.split() for x in f.read().splitlines()], dtype=np.float32)  # labels
                if len(label_i) == 0:
                    label_i = np.zeros((0, 5), dtype=np.float32)
                x[img] = [label_i, shape]
            except Exception as e:
                x[img] = [None, None]
    ...
    def __getitem__(self, index):
        ...
        if nL:
            labels[:, 1:5] = box_xyxy_to_cxcywh(labels[:, 1:5])  # convert xyxy to xywh
    ...
    def collate_fn(batch):
        ...
        for i, label_i in enumerate(label):
            label_i[:, 0] = i  # add target image index for build_targets()
