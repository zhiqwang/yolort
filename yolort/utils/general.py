...
def check_requirements(requirements='requirements.txt', exclude=(), install=True):
    ...
    if isinstance(requirements, (str, Path)):  # requirements.txt file
        ...
        requirements = [
            f'{x.name}{x.specifier}' for x in pkg.parse_requirements(file.open()) if x.name not in exclude
        ]
    ...
    if n:  # if packages updated
        ...
        s = (f"{prefix} {n} package{'s' * (n > 1)} updated per {source}\n"
             f"{prefix} ⚠️ {colorstr('bold', 'Restart runtime or rerun "
             "command for updates to take effect')}\n")
        print(emojis(s))
...
def check_imshow():
    ...
    try:
        ...
        print('WARNING: Environment does not support cv2.imshow() '
...
def coco80_to_coco91_class():  # converts 80-index (val2014) to 91-index (paper)
    ...
    x = [
        ...
        70, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81, 82, 84, 85, 86,
        87, 88, 89, 90,
    ]
    return x
...
def save_one_box(xyxy, im, file='image.jpg', gain=1.02, pad=10,
