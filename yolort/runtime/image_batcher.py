...
class ImageBatcher:
    ...
    def __init__(
        ...
        Args:
            ...
            shape: The tensor shape of the batch to prepare, either in NCHW or NHWC format.
            ...
            exact_batches: This defines how to handle a number of images that is not an exact
                multiple of the batch size. If false, it will pad the final batch with zeros
                to reach the batch size. If true, it will *remove* the last few images in excess
                of a batch size multiple, to guarantee batches are exact (useful for calibration).
        ...
        if os.path.isdir(input):
            self.images = [
                os.path.join(input, f) for f in os.listdir(input) if is_image(os.path.join(input, f))
            ]
            self.images.sort()
            ...
            raise NotImplementedError("Type of the input should be a directory or a file.")
        self.num_images = len(self.images)
        if self.num_images < 1:
            raise OSError(f"No valid {'/'.join(extensions)} images found in {input}")
        ...
        if self.num_images < 1:
            raise OSError("Not enough images to create batches")
    ...
    def preprocess_image(self, image_path):
        """
        The image preprocessor loads an image from disk and prepares it as needed
        for batching. This includes padding, resizing, normalization, data type
        casting, and transposing.
        ...
        * EfficientDet: Resizes and pads the image to fit the input size.
        ...
        def resize_pad(image, pad_color=(0, 0, 0)):
            """
            A subroutine to implement padding and resizing. This will resize the image to
            fit fully within the input size, and pads the remaining bottom-right portions
            ...
            Return:
                Two values: The PIL image object already padded and cropped, and the resize scale used.
            """
        ...
        if self.preprocessor == "EfficientDet":
            ...
            # [0-1] Normalization, Mean subtraction and Std Dev scaling are part of the
            # EfficientDet graph, so no need to do it during preprocessing here
        else:
    ...
    def get_batch(self):
        ...
        Return:
            A generator yielding three items per iteration: a numpy array holding a
            batch of images, the list of paths to the images loaded within this batch,
        ...
        for i, batch_images in enumerate(self.batches):
            ...
            print(f"BATCH SCALES: {batch_scales}")
