...
class PredictorOVO:
    ...
    def _build_ovonet(self):
        ...
        if ovo is not None:
            ...
            raise ImportError("openvino is not installed, please use command \"pip install openvino-dev\" firstly.")
        ie = IECore()
    ...
    def nms(self,outputs,iou_score=0.45,cls_score=0.2,shape=(640,640)):
        ...
        for i in range(batch):
            boxes,cls,scores = torch.from_numpy(outputs["boxes"][i]),torch.from_numpy(outputs['scores'][i].argmax(1)),torch.from_numpy(outputs['scores'][i].max(1))
