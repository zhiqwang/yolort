    ...
    def dump_args(self, operator, **kwargs):
        ...
        for arg in params.keys():
            ...
            if isinstance(params_arg, str):
                ...
                ncnn_args_phrase = (
                    f"{ncnn_args_phrase}{-23300 - arg}={len(params_arg)},"
                    f"{','.join(list(map(str, params_arg)))} "
                )
            else:
                print(arg_name, params_arg, type(params_arg))
                print(
                    f"[ERROR] Failed to dump arg {arg_name} with type {type(params_arg)}."
                )
                frameinfo = inspect.getframeinfo(inspect.currentframe())
                calframe = inspect.getouterframes(inspect.currentframe(), 2)
                print(
                    f"Failed to convert at {frameinfo.filename}:{frameinfo.lineno} "
