...
class LoadStreams:
    ...
    def __init__(self, sources="streams.txt", img_size=640, stride=32, auto=True):
        ...
        s = np.stack(
            [letterbox(x, self.img_size, stride=self.stride, auto=self.auto)[0].shape for x in self.imgs]
        )
...
def dataset_stats(path="coco128.yaml", autodownload=False, verbose=False, profile=False, hub=False):
    ...
    for split in "train", "val", "test":
        ...
        if hub:
            ...
            for _ in tqdm(
                ThreadPool(NUM_THREADS).imap(hub_ops, dataset.img_files), total=dataset.n, desc="HUB Ops"
            ):
                pass
